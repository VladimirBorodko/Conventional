...
public final class ReuseableComposer<T> {
  ...
  let view: T
  var cells: [Brief] = []
  var supplementaries: [String: [Brief]] = [:]

  var headers: [Brief] { return supplementaries[UICollectionElementKindSectionHeader] ?? [] }
  var footers: [Brief] { return supplementaries[UICollectionElementKindSectionFooter] ?? [] }

  init(_ view: T) {self.view = view}

  typealias Factory = (Brief) -> ReuseableComposer
  
  public struct Registrator<View: AnyObject> {

    let factory: Factory

    public func byClass
      ( with reuseId: String = Convention<View>.reuseIdentifier
      ) -> Configurator<View>
    { return .init(reuseId: reuseId, source: .aClass(View.self), factory: factory) }

    public func fromNib
      ( name: String = Convention<View>.nibName
      , bundle: Bundle = Convention<View>.bundle
      , with reuseId: String = Convention<View>.reuseIdentifier
      ) -> Configurator<View>
  ...
  public struct Configurator<View: AnyObject> {

    let reuseId: String
    let source: Brief.Source
    let factory: Factory
    ...
    {
      let brief = Brief(reuseId: reuseId, source: source, viewType: View.self, modelType: Model.self) { [weak owner] view, model in
    ...
    {
      let brief = Brief(reuseId: reuseId, source: source, viewType: View.self, modelType: Model.self) { view, model in
        guard let view = view as? View else { throw ViewTypeMismatch<View>(modelType: Model.self) }
        guard let model = model as? Model else { throw ModelTypeMismatch<View>(modelType: Model.self) }
        closure(view,model)
      }
